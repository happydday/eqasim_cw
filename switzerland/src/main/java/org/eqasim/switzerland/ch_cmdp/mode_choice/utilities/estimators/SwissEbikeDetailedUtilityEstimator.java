...
import org.eqasim.core.simulation.mode_choice.utilities.predictors.EbikePredictor;//NEW: Import new EbikePredictor
...
public class SwissEbikeDetailedUtilityEstimator extends BikeUtilityEstimator {
    private final SwissCmdpModeParameters parameters;
    private final SwissPersonPredictor personPredictor;
    private final EbikePredictor bikePredictor;
    //private final SwissBikePredictor bikePredictor;
    private final VariablesWriter variablesWriter;

    @Inject
    public SwissEbikeDetailedUtilityEstimator(SwissCmdpModeParameters parameters, SwissPersonPredictor personPredictor,
                                             EbikePredictor bikePredictor, VariablesWriter variablesWriter) {
    ...
    protected double estimateConstantUtility() {
        return parameters.ebike.alpha_u;
    }
    ...
    protected double estimateTravelTimeUtility(BikeVariables variables) {
        return parameters.ebike.betaTravelTime_u_min * Math.pow(variables.travelTime_min, parameters.bike.travelTimeExponent);
    }

    protected double estimateAgeUtility(SwissPersonVariables personVariables) {
        return parameters.ebike.betaAge_u * Math.max(0.0, personVariables.age_a - 18);
    }

    protected double estimateSexUtility(SwissPersonVariables personVariables) {
        return personVariables.sex==1 ? parameters.ebike.betaSex_u :0.0;
    }

    protected double estimateHomeOriginUtility(DiscreteModeChoiceTrip trip) {
        return Utils.originIsHome(trip) ? parameters.ebike.betaOriginHome_u : 0.0;
    }

    protected double estimateRegionalUtility(SwissPersonVariables personVariables) {
        if (personVariables.cantonCluster == 1) {
            return parameters.ebike.betaRegion1_u;
        } else if (personVariables.cantonCluster == 2) {
            return parameters.ebike.betaRegion2_u;
        } else {
            return 0.0;
        }
    }
    ...
    protected double estimateShortDistanceUtility(DiscreteModeChoiceTrip trip) {
        return Utils.isShortDistanceTrip(trip)? parameters.ebike.betaShortDistance_u :0.0;
    }

    protected double estimateUrbanDestinationUtility(DiscreteModeChoiceTrip trip) {
        return Utils.destinationIsUrban(trip) ? parameters.ebike.betaUrbanDestination_u : 0.0;
    }

    protected double estimateWorkDestinationUtility(DiscreteModeChoiceTrip trip) {
        return Utils.destinationIsWork(trip) ? parameters.ebike.betaDestinationWork_u : 0.0;
    }
